...
#    print(score)
    return classes
def test(model,context,target,label):
    context_combined = embedding(context)
    feature_embed_q = feature_embeding(context)
    target_combined = embedding(target)
    feature_embed_ans = feature_embeding(target)
...
def evaluate():
    ...
    question,answer,label = load_train_file('data')
    test_question,test_answer,test_label = load_train_file('test')
    
    train_result,train_acc,train_prob=test(model,question,answer,label)
    test_result,test_acc,test_prob=test(model,test_question,test_answer,test_label)
